...
public class OrderEventHandler implements EventHandler<JSONObject, OrderEventPayLoad>, EmailContentGenerator<JSONObject> {

    //~ ----------------------------------------------------------------------------------------------------------------
    //~ Instance fields 
    //~ ----------------------------------------------------------------------------------------------------------------

    private final Set<OrderEvents> orderEventsSupported = Sets.newHashSet(OrderEvents.ORDER_FILL);
    ...
    public EmailPayLoad generate(EventPayLoad<JSONObject> payLoad) {
        ...
        if (type.equals(OrderEvents.ORDER_FILL.toString())) {
            TradeableInstrument<String> instrument = new TradeableInstrument<String>(jsonPayLoad.get(OandaJsonKeys.INSTRUMENT.value()).toString());
            String accountId = (String) jsonPayLoad.get(OandaJsonKeys.ACCOUNT_ID.value());
            ...
            emailMsg = String.format("Trade event %s received for account %s. Trade Units=%s. Time=%s. Price=%s. Account balance=%s", type, accountId, tradeUnits, time.toString(), price, accountBalance);
        ...
        if (type.equals(OrderEvents.MARKET_ORDER.toString())) {
            ...
            emailMsg = String.format("Trade event %s received for account %s. Trade Units=%s. Time=%s. TakeProfit=%s. StopLoss=$s", type, accountId, tradeUnits, time.toString(), (priceTP == null) ? "0" : priceTP,
        ...
        if (type.equals(OrderEvents.TAKE_PROFIT_ORDER.toString())) {
            ...
            emailMsg = String.format("Trade event %s received for account %s. Time=%s. Price=$s", type, accountId, time.toString(), price);
