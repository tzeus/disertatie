...
import com.tudoreloprisan.strategy.FadeTheMoveStrategy;
...
public class FadeTheMoveStrategyTest {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		for (int ctr = 0; ctr < Math.min(audchfPrices.length, audjpyPrices.length); ctr++) {
			...
			eventBus.post(new MarketDataPayLoad<String>(audchf, audchfPrices[ctr] - precision,
					audchfPrices[ctr] + precision, eventStartaudchf));
			eventBus.post(new MarketDataPayLoad<String>(audjpy, audjpyPrices[ctr] - precision * 100,
					audjpyPrices[ctr] + precision * 100, eventStartaudjpy));
		}
		fadeTheMoveStrategy.analysePrices();
		for (int i = 1; i <= 2; i++) {
			TradingDecision<String> decision = orderQueue.take();
			System.out.println(String.format(
					"The com.tudoreloprisan.strategy signalled to go %s on instrument %s at limit price %2.5f and take profit %2.5f ",
