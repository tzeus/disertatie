 ...
 */
package com.tudoreloprisan.tradingAPI.util;
...
public class TradingUtils {
     ...
     * @param  request
     *
     * @return a String representation of the HttpRequest which will be or is already executed. Useful for logging the
     *         HTTP request made.
     */
     ...
     * @param  dateTime
     *
     * @return a UNIX representation of the dateTime object which actually is in fact a nanosecond representation
     *         obtained by multiplying the millisecond representation with 1000;
     */
    ...
    /**
     * Utility method to return the currency pair which form the tradeable instrument. It is effectively a string
     * tokeniser with no validation for currency or number of tokens or in fact the separator character.
     *
     * <p>input "GBP_USD", "_" will return ["GBP","USD"] input "GBP_USD", "/" will return ["GBP_USD"] input
     * "hello_GBP_USD", "_" will return ["hello","GBP","USD"]
     *
     * @param  instrument
    ...
    /**
     * A utility method that tries to toString an HttpResponse object. Only when HTTP status 200 is returned by the
     * server, will this method attempt to process the response.
     *
     * @param  response
     *
     * @return a String representation of the response if possible else an empty string.
     *
     * @throws IOException
     */
     ...
     * places in the code. Akin to StringUtils.isEmpty() which returns true if input String is null or 0 length.
     *
     * @param  collection
     *
    ...
    /**
     * A utility method that is an alternative to having to write null and empty checks for maps at various places in
     * the code. Akin to StringUtils.isEmpty() which returns true if input String is null or 0 length.
     *
     * @param  map
     *
    ...
    /**
     * A utility method that tries to split an instrument into 2 currencies. The currencies can be separated by any
     * separator such as '/','-','_' etc but must be at the 4th position in the String. The length of instrument must
     * also be exactly 7 chars.
     *
     * @param  instrument i.e. a currency pair such as GBP_USD, AUD/CHF etc
     *
     * @return exactly a pair of currencies as base and denominated.
     *
     * @throws an IllegalArgumentException if instrument cannot be split in exactly 2 currencies
     */
    ...
    /**
     * Prints error message received from an HTTP response to System.err. Should normally be called when the HTTP status
     * <> 200 in order to print out the cause for non 200 status code.
     *
     * @param  response
     *
    ...
    public static final int getSign(String currencyInstrument, TradingSignal side, String currency) {
        ...
        if (currencyInstrument.contains(currency)) {
            ...
            if ((currency.equals(ccyPairs[0]) && (side == TradingSignal.LONG)) || (currency.equals(ccyPairs[1]) && (side == TradingSignal.SHORT))) {
                return 1;
            } else {
                return -1;
            }
