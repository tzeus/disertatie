	...
	private Account<String> getLatestAccountInfo(final String accountId, CloseableHttpClient httpClient) {
		try {
			...
			if (strResp != StringUtils.EMPTY) {
				...
				final double accountBalance = (Double.valueOf((String) accountJson.get(BrokerJsonKeys.BALANCE.value()))).doubleValue();
				final double accountUnrealizedPnl = (Double.valueOf((String) accountJson.get(BrokerJsonKeys.UNREALIZED_PL.value()))).doubleValue();
				String realizedPL = (String) accountJson.get(BrokerJsonKeys.REALIZED_PL.value());
				final double accountRealizedPnl = (realizedPL!=null?Double.valueOf(realizedPL).doubleValue():0);
				final double accountMarginUsed = (Double.valueOf((String) accountJson.get(BrokerJsonKeys.MARGIN_USED.value()))).doubleValue();
				final double accountMarginAvailable = (Double.valueOf((String) accountJson.get(BrokerJsonKeys.MARGIN_AVAIL.value()))).doubleValue();
				final Long accountOpenTrades = (Long) accountJson.get(BrokerJsonKeys.OPEN_TRADES.value());
				final String accountBaseCurrency = (String) accountJson.get(BrokerJsonKeys.ACCOUNT_CURRENCY.value());
				final Double accountLeverage = Double.valueOf((String) accountJson.get(BrokerJsonKeys.MARGIN_RATE.value()));
	...
	public Collection<Account<String>> getLatestAccountsInfo() {
		...
		try {
			...
			if (strResp != StringUtils.EMPTY) {
				...
				JSONArray accounts = (JSONArray) jsonResp.get(BrokerJsonKeys.ACCOUNTS.value());
				...
				for (Object o : accounts) {
					...
					String accountIdentifier = (String) account.get(BrokerJsonKeys.ACCOUNT_ID.value());
